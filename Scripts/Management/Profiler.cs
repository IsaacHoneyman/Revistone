    ...
    static void ProfileBehaviour(int tickNum)
    {
        ...
        if (tickNum % 20 == 0)
        {
            ...
            if (Enabled)
            {
                ...
                UpdateDebugConsoleLine(new ConsoleLine($"[Profiler] FPS: {Fps}, TPS: {Tps}, Mode: {(use24BitColourRendering ? "Quality" : "Performance")}", BuildArray(AppRegistry.SecondaryCol.SetLength(10), AppRegistry.PrimaryCol)), debugStartIndex + 1);
                ...
                UpdateDebugConsoleLine(new ConsoleLine(RenderLogicTime.Count == 0 ? "Render Mode - Single Threaded" : $"Render Frames (System Ticks): Avg - {RenderLogicTime.Sum() / RenderLogicTime.Count} | Target - {displayWindowsTickInterval}", AppRegistry.PrimaryCol), debugStartIndex + 5);
            }
